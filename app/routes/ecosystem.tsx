...
const Card = ({ app }: { app: App }) => {
  return (
    <a
      className="flex flex-col rounded-lg border-2 border-honey-300 bg-honey-50 px-4 py-3 leading-normal transition-all hover:scale-[1.01] hover:border-honey-500 hover:bg-honey-50 md:px-9 md:py-8"
    ...
    >
      <div className="flex flex-row justify-between">
        <img
          className="flex w-[64px] flex-row rounded-md bg-honey-100"
          src={app.image}
          alt={`${app.name} Logo`}
        />
...
export default function Team() {
  ...
  return (
    <Layout>
      <main>
        <section className="relative bg-night-900 pb-16 pt-32 sm:pb-24 sm:pt-48">
        ...
        <section className="bg-honey-100">
          <div className="relative mx-auto max-w-3xl space-y-4 py-16 px-8 sm:py-24 sm:px-6 lg:max-w-9xl lg:px-12">
            <div className="flex items-center justify-between gap-4">
              <p className="flex items-center text-2xl font-bold leading-normal text-ruby-900">
                Ecosystem Projects
                <span className="ml-3 w-max rounded-lg border-honey-700 bg-honey-400 px-2.5 py-1 font-mono text-[0.75rem] font-medium uppercase text-ruby-900">
                  {ecosystem.length}
                </span>
              </p>
              ...
              <div className="flex gap-4">
                ...
                >
                  <SelectTrigger className="w-max space-x-1.5 border-honey-700 bg-honey-300 text-night-900">
                    <SelectValue placeholder="Select a type" />
                  </SelectTrigger>
                  <SelectContent
                    align="end"
                    className="border-honey-700 bg-honey-200 p-2 text-new-night-700 shadow-honey-100/10"
                  >
