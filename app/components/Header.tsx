...
export const Header = ({ openModal }: { openModal: () => void }) => {
  ...
  return (
    <>
      <Popover as="header" className="sticky top-0 z-50 lg:relative">
          ...
          <div className="relative mx-auto flex max-w-9xl items-center justify-center px-4 lg:py-8 lg:px-0">
            ...
            <Popover.Group as="nav" className="hidden space-x-6 lg:flex">
              {navigation.map((item) => {
                if (item.links) {
                  return (
                    <Popover className="relative z-40" key={item.name}>
                      {({ open }) => (
                        <>
        ...
        >
          <Popover.Panel
            focus
            className="absolute inset-x-0 top-0 z-30 origin-top transform transition lg:hidden"
          >
      ...
      {isLeafPage ? (
        <div className="sticky top-0 z-30 hidden bg-ruby-900 py-3.5 lg:block">
